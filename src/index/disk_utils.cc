...
void add_new_file_to_single_index(
    std::string index_file, std::string new_file) {
  ...
  for (uint64_t i = 0; i < nr; i++) new_meta.push_back(metadata[i]);
...
size_t calculate_num_pq_chunks(
    double final_index_ram_limit, size_t points_num, uint32_t dim,
    const std::vector<std::string> &param_list) {
  size_t num_pq_chunks = (size_t)(std::floor)(
      uint64_t(final_index_ram_limit / (double)points_num));
...
T *generateRandomWarmup(
  ...
  diskann::alloc_aligned(
      ((void **)&warmup), warmup_num * warmup_aligned_dim * sizeof(T),
      8 * sizeof(T));
...
T *load_warmup(
  ...
  if (files.fileExists(cache_warmup_file)) {
    diskann::load_aligned_bin<T>(
        files, cache_warmup_file, warmup, warmup_num, file_dim,
        file_aligned_dim);
...
T *load_warmup(
  ...
  if (file_exists(cache_warmup_file)) {
    diskann::load_aligned_bin<T>(
        cache_warmup_file, warmup, warmup_num, file_dim, file_aligned_dim);
    if (file_dim != warmup_dim || file_aligned_dim != warmup_aligned_dim) {
      std::stringstream stream;
      stream << "Mismatched dimensions in sample file. file_dim = " << file_dim
             << " file_aligned_dim: " << file_aligned_dim
...
int merge_shards(
    ...
    bool use_filters, const std::string &labels_to_medoids_file) {
  // Read ID maps
  ...
  for (uint64_t shard = 0; shard < nshards; shard++) {
    ...
    read_idmap(
        idmaps_prefix + std::to_string(shard) + idmaps_suffix, idmaps[shard]);
  }
    ...
    for (size_t i = 0; i < nshards; i++) {
      ...
      while (std::getline(mapping_reader, line)) {
        ...
        while (std::getline(iss, token, ',')) {
          ...
          token.erase(
              std::remove(token.begin(), token.end(), '\r'), token.end());
  ...
  cached_ofstream merged_vamana_writer(
      output_vamana, BUFFER_SIZE_FOR_CACHED_IO);
  ...
  for (const auto &id_shard : node_shard) {
    ...
    if (cur_id < node_id) {
      ...
      merged_vamana_writer.write(
          (char *)final_nhood.data(), nnbrs * sizeof(uint32_t));
      ...
      cur_id = node_id;
      nnbrs = 0;
      for (auto &p : final_nhood) nhood_set[p] = 0;
      final_nhood.clear();
    }
    ...
    if (shard_nnbrs == 0) {
      diskann::cout << "WARNING: shard #" << shard_id << ", node_id " << node_id
                    << " has 0 nbrs" << std::endl;
      abort();
    }
    ...
    if (shard_nnbrs > 0)
      vamana_readers[shard_id].read(
          (char *)shard_nhood.data(), shard_nnbrs * sizeof(uint32_t));
    // rename nodes
  ...
  merged_vamana_writer.close();
  // std::ifstream merged_vamana_reader(output_vamana, std::ios::in |
  // std::ios::binary); std::ofstream
  // mem_layout_converter(std::string(output_vamana + "mem_layout"),
  // std::ios::out | std::ios::binary);

  // merged_vamana_reader.read((char *)&merged_index_size, sizeof(uint64_t));
  // mem_layout_converter.write((char *)&merged_index_size, sizeof(uint64_t));

  // merged_vamana_reader.read((char *)&output_width, sizeof(uint32_t));
  // mem_layout_converter.write((char *)&output_width, sizeof(uint32_t));
...
template <typename T>
void breakup_dense_points(
    const std::string data_file, const std::string labels_file,
  ...
  if (labels_stream.is_open()) {
    while (getline(labels_stream, line)) {
      ...
      while (getline(iss, token, ',')) {
        if (lbl_cnt == density) {
          if (label_host == point_cnt) dense_pts++;
  ...
  if (dummy_pt_ids.size() != 0) {
    ...
    T *ptr = (T *)std::realloc(
        (void *)data, labels_per_point.size() * ndims * sizeof(T));
    if (ptr == nullptr) {
      ...
      throw new diskann::ANNException(
          "Realloc failed while expanding data.", -1, __FUNCTION__, __FILE__,
          __LINE__);
    } else {
      data = ptr;
    }
    ...
    for (auto i = dummy_pt_ids.begin(); i != dummy_pt_ids.end(); i++) {
      ...
      std::memcpy(
          data + i->first * ndims, data + i->second * ndims, ndims * sizeof(T));
    }
    dummy_writer.close();
  }
...
int build_merged_vamana_index(
  ...
  double full_index_ram =
      estimate_ram_usage(base_num, (uint32_t)base_dim, sizeof(T), R);
  ...
  std::string merged_meta_index_prefix = merged_index_prefix + "_meta_index";

  Timer timer;

#ifdef TARGET_PARTS
  int target_parts = TARGET_PARTS;
  partition_with_target_parts<T>(
      base_file, sampling_rate, ram_budget, merged_index_prefix,
      NUM_REPLICA, target_parts, true);
  int num_parts = target_parts;
  std::cout << "====Target parts: " << target_parts
            << " Number of replicas: " << NUM_REPLICA << std::endl;
#else
  int num_parts = partition_with_ram_budget<T>(
      base_file, sampling_rate, ram_budget, merged_index_prefix, 1);
#endif
  ...
  // float meta_ratio = meta_data_ratio;
  // two layer k-means, sample data from each partition

  std::cout << "====Begin to Build Partitioned Indices====" << std::endl;
  // exit(1);
  for (int p = 0; p < num_parts; p++) {
    ...
    // std::string shard_base_file = merged_index_prefix + "_subshard-" +
    // std::to_string(p) + ".bin";

    // std::string shard_ids_file = merged_index_prefix + "_partitioned_meta" +
    // "_subshard-" + std::to_string(p) +
    // "_ids_uint32.bin"; std::string shard_ids_file = merged_index_prefix +
    // "_partitioned_meta_replica_" + std::to_string(num_replica) + "_subshard-"
    // + std::to_string(p) + "_ids_uint32.bin";

    // build partitioned by k-means
    ...
    // std::string shard_index_file = merged_index_prefix + "_partitioned_meta"
    // + "_subshard-" + std::to_string(p) +
    // "_mem.index";
    std::string shard_index_file =
        merged_index_prefix + "_subshard-" + std::to_string(p) + "_mem.index";

    diskann::IndexWriteParameters low_degree_params =
        diskann::IndexWriteParametersBuilder(L, 2 * R / 3)
            .with_filter_list_size(Lf)
    ...
    diskann::Index<T> _index(
        compareMetric, shard_base_dim, shard_base_pts,
        std::make_shared<diskann::IndexWriteParameters>(low_degree_params),
        nullptr, defaults::NUM_FROZEN_POINTS_STATIC, false, false, false,
    ...
    //     shard_labels_file); if (universal_label != "") { //  indicates no
    //     universal label
    //         LabelT unv_label_as_num = 0;
    //         _index.set_universal_label(unv_label_as_num);
    //     }
    //     _index.build_filtered_index(shard_base_file.c_str(),
    //     shard_labels_file, shard_base_pts);
    // }
    ...
    // partition the shards to wanted number of parts, aiming to get the
    // centroids. sample shards ids first, read data to memory, and then run
    // kmeans partition save the centorids (deduplicate) and its data build the
    // meta index and save the graph

    std::remove(shard_base_file.c_str());
  }

  diskann::cout << timer.elapsed_seconds_for_step("building indices on shards")
                << std::endl;

  timer.reset();
  diskann::merge_shards(
      merged_index_prefix + "_subshard-", "_mem.index",
      merged_index_prefix + "_subshard-", "_ids_uint32.bin", num_parts, R,
      mem_index_path, medoids_file, use_filters, labels_to_medoids_file);
  // diskann::merge_shards(merged_index_prefix + "_partitioned_meta" +
  // "_subshard-", "_mem.index", merged_index_prefix
  // + "_partitioned_meta" + "_subshard-",
  //                       "_ids_uint32.bin", num_parts, R, mem_index_path,
  //                       medoids_file, use_filters, labels_to_medoids_file);
  ...
  for (int p = 0; p < num_parts; p++) {
    ...
    // std::string shard_ids_for_merge_build_file = merged_index_prefix +
    // "_partitioned_meta_replica_" + std::to_string(num_replica) + "_subshard-"
    // + std::to_string(p) + "_ids_uint32.bin";
    ...
    // std::remove(shard_ids_for_merge_build_file.c_str());
    if (use_filters) {
...
template <typename T, typename LabelT>
int kmeans_partition(IndexParameter &index_param) {
  std::cout << "Executing kmeans_partition()" << std::endl;
  if (!std::is_same<T, float>::value &&
      (index_param.metric == diskann::Metric::INNER_PRODUCT ||
       index_param.metric == diskann::Metric::COSINE)) {
    std::stringstream stream;
  ...
  // disk needed!
  if (index_param.metric == diskann::Metric::INNER_PRODUCT) {
    Timer timer;
    std::cout << "Using Inner Product search, so need to pre-process base "
                 ...
                 "final index."
              << std::endl;
    index_param.data_file_to_use = index_param.prepped_base;
    float max_norm_of_base = diskann::prepare_base_for_inner_products<T>(
        index_param.data_path, index_param.prepped_base);
    std::string norm_file = index_param.disk_index_path + "_max_base_norm.bin";
    ...
    diskann::cout << timer.elapsed_seconds_for_step(
                  ...
                  << std::endl;
    index_param.created_temp_file_for_processed_data = true;
  } else if (index_param.metric == diskann::Metric::COSINE) {
    Timer timer;
    std::cout << "Normalizing data for cosine to temporary file, please ensure "
              ...
              << std::endl;
    index_param.data_file_to_use = index_param.prepped_base;
    diskann::normalize_data_file(
        index_param.data_path, index_param.prepped_base);
    diskann::cout << timer.elapsed_seconds_for_step(
                         "preprocessing data for cosine")
                  << std::endl;
    index_param.created_temp_file_for_processed_data = true;
  }

  double final_index_ram_limit = get_memory_budget(index_param.B);
  ...
  double indexing_ram_budget = index_param.M;
  if (indexing_ram_budget <= 0) {
    std::cerr << "Not building index. Please provide more RAM budget"
              << std::endl;
    return -1;
  }
  uint32_t num_threads = index_param.num_threads;
  ...
  diskann::get_bin_metadata(
      index_param.data_file_to_use.c_str(), points_num, dim);
  // here we change to fixed sample size.
  const double p_val = ((double)(500000) / (double)points_num);
...
// This code should only be available in the "build" DLL.
  ...
  size_t base_num, base_dim;
  diskann::get_bin_metadata(index_param.data_path, base_num, base_dim);

  double full_index_ram = estimate_ram_usage(
      base_num, (uint32_t)base_dim, sizeof(T), index_param.max_degree);

  Timer timer_partition;

#ifdef TARGET_PARTS
  int target_parts = TARGET_PARTS;
  partition_with_target_parts<T>(
      index_param, p_val, indexing_ram_budget, target_parts, true);
  int num_parts = target_parts;
  std::cout << "====Target parts: " << target_parts
            << " Number of replicas: " << index_param.replica_num << std::endl;
#else
  int num_parts = partition_with_ram_budget<T>(
      index_param.data_path, p_val, ram_budget, 2 * R / 3, merged_index_prefix,
      1);
#endif
  diskann::cout << timer_partition.elapsed_seconds_for_step(
                       "partitioning data ")
                << std::endl;

  std::string cur_centroid_filepath =
      index_param.merged_index_prefix + "_centroids.bin";
  std::rename(
      cur_centroid_filepath.c_str(), index_param.centroids_path.c_str());

  timer_partition.reset();

  /**
   * Split base to parititons.
   */

  // TODO: Put this part to build.
  std::cout << "====Begin to Retrieve Replica Partitioned Data===="
            << std::endl;
  // exit(1);
  printf("<<>> size ofT %u\n", sizeof(T));
  retrieve_shard_data_from_ids<T>(index_param, index_param.replica_num);
//   for (int p = 0; p < num_parts; p++) {
// #if defined(DISKANN_RELEASE_UNUSED_TCMALLOC_MEMORY_AT_CHECKPOINTS) && \
//     defined(DISKANN_BUILD)
//     MallocExtension::instance()->ReleaseFreeMemory();
...
//     // partition replica data
//     retrieve_shard_data_from_ids<T>(
//         index_param.data_path, index_param.part_replica_idmap_filename[p],
//         index_param.part_replica_base_filename[p]);

//     // partition no-replica data
//     retrieve_shard_data_from_ids<T>(
//         index_param.data_path, index_param.part_noreplica_idmap_filename[p],
//         index_param.part_noreplica_base_filename[p]);
//     // std::remove(shard_base_file.c_str());
//   }
  
  diskann::cout << timer.elapsed_seconds_for_step("partition with kmeans time ")
                << std::endl;

  timer.reset();

  // /**
  //  * Start to scatter and gather shard data.
  //  */
  ...
  //                               std::to_string(p) + "_ids_uint32.bin";
  //   std::string shard_labels_file =
  //       merged_index_prefix + "_subshard-" + std::to_string(p) +
  //       "_labels.txt";
  //   std::string shard_index_file =
  //       merged_index_prefix + "_subshard-" + std::to_string(p) +
  //       "_mem.index";
  //   std::string shard_index_file_data = shard_index_file + ".data";
  //   // std::string shard_ids_for_merge_build_file = merged_index_prefix +
  //   // "_partitioned_meta_replica_" + std::to_string(num_replica) +
  //   // "_subshard-"
  //   // + std::to_string(p) + "_ids_uint32.bin";
...
/**
 * For baseline2 random range partition.
 */
template <typename T, typename LabelT>
int random_partition(IndexParameter &index_param) {
  std::cout << "Executing random_partition()" << std::endl;
  if (!std::is_same<T, float>::value &&
      (index_param.metric == diskann::Metric::INNER_PRODUCT ||
       index_param.metric == diskann::Metric::COSINE)) {
    std::stringstream stream;
...
// This code should only be available in the "build" DLL.
...
#ifdef TARGET_PARTS
  int target_parts = TARGET_PARTS;
  random_slice_parts<T>(
      index_param, target_parts);
  int num_parts = target_parts;
  std::cout << "====Target parts: " << target_parts
            << " Number of replicas: " << NUM_REPLICA << std::endl;
#else
  ...
  // TODO: Put this part to build.
  std::cout << "====Begin to Retrieve Partitioned Data====" << std::endl;
  // exit(1);
  for (int p = 0; p < num_parts; p++) {
#if defined(DISKANN_RELEASE_UNUSED_TCMALLOC_MEMORY_AT_CHECKPOINTS) && \
    defined(DISKANN_BUILD)
    ...
    // build partitioned by k-means
    // index_param.shard_base_file = index_param.merged_index_prefix +
    //                               "_subshard-" + std::to_string(p) + ".bin";

    // index_param.shard_ids_file = index_param.merged_index_prefix +
    //                              "_subshard-" + std::to_string(p) +
    //                              "_ids_uint32.bin";

    // index_param.shard_labels_file = index_param.merged_index_prefix +
    //                                 "_subshard-" + std::to_string(p) +
    //                                 "_labels.txt";


    // NOTE: To unify random and kmeans partition, for the random partition
    // we use the part_replica_idmap_filename and part_replica_base_filename 
    // to store the partition id and vector. So, both file is actually no-replica.
    retrieve_shard_data_from_ids<T>(
        index_param.data_path, index_param.part_noreplica_idmap_filename[p],
        index_param.part_noreplica_base_filename[p]);

    // index_param.shard_index_file = index_param.merged_index_prefix +
    //                                "_subshard-" + std::to_string(p) +
    //                                "_mem.index";
  }
  // build partitioned by k-means
  // reset to local index param.
  // index_param.shard_base_file = index_param.merged_index_prefix +
  // "_subshard-" +
  //                               std::to_string(0) + ".bin";
  // index_param.shard_ids_file = index_param.merged_index_prefix + "_subshard-"
  // +
  //                              std::to_string(0) + "_ids_uint32.bin";
  // index_param.shard_labels_file = index_param.merged_index_prefix +
  //                                 "_subshard-" + std::to_string(0) +
  //                                 "_labels.txt";
  // index_param.shard_index_file = index_param.merged_index_prefix +
  //                                "_subshard-" + std::to_string(0) +
  //                                "_mem.index";
...
template <typename T, typename LabelT>
void build_all_partition(IndexParameter &index_param){
  diskann::cout << "Building all partition in local, thread num: "
                << index_param.num_threads << std::endl;
  uint32_t num_threads = index_param.num_threads;
  ...
  uint32_t partition_degree = index_param.graph_type == SharedNothing
                                  ? index_param.max_degree
                                  : (2 * index_param.max_degree / 3);

  diskann::IndexWriteParameters low_degree_params =
      diskann::IndexWriteParametersBuilder(index_param.L, partition_degree)
          .with_filter_list_size(index_param.Lf)
          .with_saturate_graph(false)
          .with_num_threads(index_param.num_threads)
          .build();

  for(uint32_t p = 0; p <index_param.num_parts; p++){
    uint64_t shard_base_dim, shard_base_pts;
    get_bin_metadata(
        index_param.part_replica_base_filename[p], shard_base_pts, shard_base_dim);

    diskann::Index<T> _index(
        index_param.metric, shard_base_dim, shard_base_pts,
        std::make_shared<diskann::IndexWriteParameters>(low_degree_params),
        nullptr, defaults::NUM_FROZEN_POINTS_STATIC, false, false, false,
        index_param.build_pq_bytes > 0, index_param.build_pq_bytes,
        index_param.use_opq);
    if (!index_param.use_filters) {
      _index.build(index_param.part_replica_base_filename[p].c_str(), shard_base_pts);
    }
    _index.save(index_param.part_replica_index_filename[p].c_str());

    diskann::cout << "Local index save to: " << index_param.part_replica_index_filename[p]
                  << std::endl;
    // copy universal label file from first shard to the final destination
    // index, since all shards anyway share the universal label
    if (index_param.machine_id == 0) {
      std::string shard_universal_label_file =
          index_param.part_replica_index_filename[p] + "_universal_label.txt";
      if (index_param.universal_label != "") {
        copy_file(
            shard_universal_label_file,
            index_param.final_index_universal_label_file);
      }
    }
    std::remove(index_param.part_replica_base_filename[p].c_str());
  }
  return ;
}

template <typename T, typename LabelT>
void build_local_partition(IndexParameter &index_param) {
  diskann::cout << "Building local partition, thread num: "
                << index_param.num_threads << std::endl;
  uint32_t num_threads = index_param.num_threads;
  ...
  if(index_param.replica_num > 1){
    get_bin_metadata(
      index_param.local_replica_base_file, shard_base_pts, shard_base_dim);
  }
  else{
    get_bin_metadata(
      index_param.local_noreplica_base_file, shard_base_pts, shard_base_dim);
  }
  ...
  if (!index_param.use_filters) {
    if(index_param.replica_num > 1){
      _index.build(index_param.local_replica_base_file.c_str(), shard_base_pts);
    }
    else{
      _index.build(index_param.local_noreplica_base_file.c_str(), shard_base_pts);
    }
  }
  _index.save(index_param.local_index_file.c_str());

  diskann::cout << "Local index save to: " << index_param.local_index_file
                << std::endl;
  // copy universal label file from first shard to the final destination
  // index, since all shards anyway share the universal label
  if (index_param.machine_id == 0) {
    std::string shard_universal_label_file =
        index_param.local_index_file + "_universal_label.txt";
    if (index_param.universal_label != "") {
      copy_file(
          shard_universal_label_file,
          index_param.final_index_universal_label_file);
...
void collect_replica_partition(IndexParameter &index_param) {
  const uint64_t nshards = index_param.num_parts;
  uint32_t max_degree = index_param.max_degree;
  // const std::string medoids_file = index_param.medoids_path;
  bool use_filters = index_param.use_filters;

  // NOTE: assume all node is sorted now.
  // get memoid
  if(!std::filesystem::exists(index_param.local_index_file)){
    std::cerr<<"Error: local index file does not exist.\n ";
    abort();
  }
  std::ifstream local_index_reader(
      index_param.local_index_file, std::ios::binary);
  size_t expected_file_size;
  local_index_reader.read((char *)&expected_file_size, sizeof(uint64_t));

  size_t index_metadata_size =
      sizeof(uint64_t) + sizeof(uint32_t) + sizeof(uint32_t) +
      sizeof(uint64_t);  // expected file size + max degree +
                         // medoid_id + frozen_point info

  uint32_t input_width;
  local_index_reader.read((char *)&input_width, sizeof(uint32_t));

  std::ofstream medoid_writer(
      index_param.medoids_path.c_str(), std::ios::binary);
  uint32_t nshards_u32 = (uint32_t)nshards;
  uint32_t one_val = 1;
  medoid_writer.write((char *)&nshards_u32, sizeof(uint32_t));
  medoid_writer.write((char *)&one_val, sizeof(uint32_t));

  // as of now the functionality to merge many overlapping vamana
  // indices is supported only for bulk indices without frozen point.
  // Hence the final index will also not have any frozen points.
  uint64_t final_index_frozen = 0;

  // read medoid
  local_index_reader.read((char *)&index_param.medoid, sizeof(uint32_t));
  local_index_reader.read((char *)&final_index_frozen, sizeof(uint64_t));
  assert(final_index_frozen == false);
  std::cout << "this partition enterpoint is " << index_param.medoid << "\n";
  // rename medoid
  index_param.medoid = index_param.local_replica_ids[index_param.medoid];

  medoid_writer.write((char *)&index_param.medoid, sizeof(uint32_t));
  // write renamed medoid
  // merged_index_writer.write((char *)&index_param.medoid, sizeof(uint32_t));

  // merged_index_writer.write((char *)&merged_index_frozen, sizeof(uint64_t));
  medoid_writer.close();

  diskann::cout << "Starting dispatch & collect ..." << std::endl;

  // start to compute dispatch partiton metadata.
  for (uint32_t m = 0; m < MACHINE_NUM; m++) {
    // get the num of ids and size of partition.
    index_param.dispatch_id_num[m] = 0;
    // index_param.local_part2id_map[m].size();
    index_param.dispatch_ngh_num[m] = 0;
  }

  size_t nelems = index_param.local_replica_ids.size();
  for (uint32_t i = 0; i < nelems; i++) {
    uint32_t v = index_param.local_replica_ids[i];
    uint32_t deg = 0;
    local_index_reader.read((char *)&deg, sizeof(uint32_t));
    std::vector<uint32_t> nghs(deg);
    if (deg > 0) {
      local_index_reader.read((char *)nghs.data(), deg * sizeof(uint32_t));
    } else {
      std::cout << "Error: some vector with 0 ngh.\n";
      abort();
    }
    uint32_t p = index_param.kmeans_id2part_map[v];
    index_param.dispatch_id_num[p]++;
    index_param.dispatch_ngh_num[p] += (size_t)(deg + 1);
  }

  // alloc ngh memory for each dispatch partition.
  for (uint32_t m = 0; m < MACHINE_NUM; m++) {
    index_param.dispatch_ids[m] =
        (char *)malloc(index_param.dispatch_id_num[m] * sizeof(uint32_t));
    index_param.dispatch_nghs[m] =
        (char *)malloc(index_param.dispatch_ngh_num[m] * sizeof(uint32_t));
    printf("dispatch m%u id num %llu ngh num %llu\n", m, 
      index_param.dispatch_id_num[m], index_param.dispatch_ngh_num[m]);
  }
  ...
  for (uint32_t i = 0; i < nelems; i++) {
    uint32_t v = index_param.local_replica_ids[i];
    uint32_t p = index_param.kmeans_id2part_map[v];
    memcpy(index_param.dispatch_ids[p] + id_offset[p], &v, sizeof(uint32_t));
    id_offset[p] += sizeof(uint32_t);

    local_index_reader.read(
        (char *)(index_param.dispatch_nghs[p] + ngh_offset[p]),
        sizeof(uint32_t));
    uint32_t deg = *(uint32_t *)(index_param.dispatch_nghs[p] + ngh_offset[p]);
    ngh_offset[p] += sizeof(uint32_t);

    if (deg > 0) {
      uint32_t remap_ngh[deg];
      local_index_reader.read((char *)remap_ngh, deg * sizeof(uint32_t));
      for (uint32_t j = 0; j < deg; j++) {
        remap_ngh[j] = index_param.local_replica_ids[remap_ngh[j]];
      }
      memcpy(
          index_param.dispatch_nghs[p] + ngh_offset[p], remap_ngh,
          deg * sizeof(uint32_t));
      ngh_offset[p] += deg * sizeof(uint32_t);
    }
  }

  local_index_reader.close();

  return;
}

void merge_replica_partition(IndexParameter &index_param) {
  // Read ID maps
  // std::vector<std::string> vamana_names(nshards);
  // std::vector<std::vector<uint32_t>> idmaps(nshards);
  // for (uint64_t shard = 0; shard < nshards; shard++) {
  //   vamana_names[shard] = vamana_prefix + std::to_string(shard) +
  //   vamana_suffix; read_idmap(
  //       idmaps_prefix + std::to_string(shard) + idmaps_suffix,
  //       idmaps[shard]);
  // }

  size_t collect_id_num = 0;
  for (uint32_t m = 0; m < MACHINE_NUM; m++) {
    collect_id_num += index_param.collect_id_num[m];
  }
  ...
  // nnodes++;
  // diskann::cout << "# nodes: " << nnodes << ", max. degree: " << max_degree
  //               << ", # nodeselems(include replica): " << nelems <<
  //               std::endl;
  // Gopal. random_shuffle() is deprecated.

  // compute inverse map: node -> shards
  std::vector<std::pair<uint32_t, uint32_t>> node_shard;
  node_shard.reserve(collect_id_num);
  std::cout << "collect_id_num: " << collect_id_num << std::endl;
  for (uint32_t m = 0; m < MACHINE_NUM; m++) {
    diskann::cout << "Creating inverse map -- shard #" << m << std::endl;
    uint32_t *collect_ids = (uint32_t *)index_param.collected_ids[m];
    for (size_t idx = 0; idx < index_param.collect_id_num[m]; idx++) {
      node_shard.push_back(std::make_pair(collect_ids[idx], (uint32_t)m));
    }
  }
  std::cout << "node_shard_size: " << node_shard.size() << std::endl;
  std::sort(
      node_shard.begin(), node_shard.end(),
      [](const auto &left, const auto &right) {
        return left.first < right.first ||
  ...
  // create cached vamana writers
  cached_ofstream merged_vamana_writer(
      index_param.final_index_path, BUFFER_SIZE_FOR_CACHED_IO);
  ...
  uint32_t max_degree = index_param.max_degree;
  diskann::cout << "max_degree: " << max_degree << std::endl;
  merged_vamana_writer.write((char *)&max_degree, sizeof(uint32_t));

  merged_vamana_writer.write((char *)&index_param.medoid, sizeof(uint32_t));
  size_t merged_index_frozen = 0;
  merged_vamana_writer.write((char *)&merged_index_frozen, sizeof(uint64_t));
  ...
  std::vector<bool> ngh_set(index_param.max_vid, 0);
  std::vector<uint32_t> final_ngh;
  ...
  uint32_t vid_cnt = 0;
  uint32_t cur_id = node_shard[0].first;
  for (const auto &id_shard : node_shard) {
    uint32_t vid = id_shard.first;
    uint32_t pid = id_shard.second;
    if (cur_id < vid) {
      // Gopal. random_shuffle() is deprecated.

      std::shuffle(final_ngh.begin(), final_ngh.end(), urng);
      final_deg = (uint32_t)(std::min)(final_ngh.size(), (uint64_t)max_degree);
      if (final_deg > 0) {
        // write into merged ofstream
        merged_vamana_writer.write((char *)&final_deg, sizeof(uint32_t));
        merged_vamana_writer.write(
            (char *)final_ngh.data(), final_deg * sizeof(uint32_t));
        merged_index_size += (sizeof(uint32_t) + final_deg * sizeof(uint32_t));
        if (cur_id % 499999 == 1) {
          diskann::cout << "." << std::flush;
        }
        cur_id = vid;
        final_deg = 0;
        for (auto &n : final_ngh) ngh_set[n] = 0;
        final_ngh.clear();
        vid_cnt++;
      } else {
        std::cout << "Error: final deg is zero...\n";
        abort();
      }
    }
    // read from pid ifstream
    deg = p_nghs_ptr[pid][p_offset[pid]];
    p_offset[pid]++;
    // vamana_readers[pid].read((char *)&shard_nnbrs, sizeof(uint32_t));

    if (deg == 0) {
      diskann::cout << "WARNING: pid #" << pid << ", vid " << vid
                    << " has 0 nghs" << std::endl;
      abort();
    }

    if (deg > 0) {
      for (uint64_t d = p_offset[pid]; d < p_offset[pid] + deg; d++) {
        // no need to remap
        uint32_t ngh_id = p_nghs_ptr[pid][d];
        if (ngh_set[ngh_id] == 0) {
          ngh_set[ngh_id] = 1;
          final_ngh.emplace_back(ngh_id);
        }
      }
      p_offset[pid] += deg;
    }
  ...
  if (final_deg > 0) {
    ...
    merged_index_size += (sizeof(uint32_t) + final_deg * sizeof(uint32_t));
    for (auto &p : final_ngh) ngh_set[p] = 0;
    final_ngh.clear();
    vid_cnt++;
  } else {
    std::cout << "Error: final deg is zero...\n";
  ...
  diskann::cout << "vid_cnt: " << vid_cnt << std::endl;
  ...
  for(uint32_t m = 0;m < MACHINE_NUM;m++){
    ...
    if(index_param.dispatch_nghs[m]!=nullptr) free(index_param.dispatch_nghs[m]);
    if(index_param.dispatch_ids[m]!=nullptr) free(index_param.dispatch_ids[m]);
    if(m == index_param.machine_id) continue;
    if(index_param.collected_nghs[m]!=nullptr) free(index_param.collected_nghs[m]);
    if(index_param.collected_ids[m]!=nullptr) free(index_param.collected_ids[m]);
  }

  return;
}

void delete_temp_file(IndexParameter &index_param) {
  // delete tempFiles
  for (int p = 0; p < index_param.num_parts; p++) {
    std::string shard_base_file = index_param.merged_index_prefix +
                                  "_subshard-" + std::to_string(p) + ".bin";
    std::string shard_id_file = index_param.merged_index_prefix + "_subshard-" +
                                std::to_string(p) + "_ids_uint32.bin";
    std::string shard_labels_file = index_param.merged_index_prefix +
                                    "_subshard-" + std::to_string(p) +
                                    "_labels.txt";
    std::string shard_index_file = index_param.merged_index_prefix +
                                   "_subshard-" + std::to_string(p) +
                                   "_mem.index";
    std::string shard_index_file_data = shard_index_file + ".data";
    // std::string shard_ids_for_merge_build_file = merged_index_prefix +
...
void create_disk_layout(
  ...
  if (reorder_data_file != std::string("")) {
    ...
    reorder_data_reader.exceptions(
    ...
    try {
      ...
      if (npts_reorder_file != npts)
        throw ANNException(
            "Mismatch in num_points between reorder "
      ...
      if (reorder_data_file_size != 8 + sizeof(float) *
        ...
        throw ANNException(
            "Discrepancy in reorder data file size ", -1, __FUNCSIG__, __FILE__,
            __LINE__);
    } catch (std::system_error &e) {
      throw FileException(
          reorder_data_file, e, __FUNCSIG__, __FILE__, __LINE__);
    }
  }
  ...
  if (vamana_frozen_num == 1) vamana_frozen_loc = medoid;
  max_node_len =
      (((uint64_t)width_u32 + 1) * sizeof(uint32_t)) + (ndims_64 * sizeof(T));
  nnodes_per_sector =
      defaults::SECTOR_LEN / max_node_len;  // 0 if max_node_len > SECTOR_LEN
  ...
  std::unique_ptr<char[]> sector_buf =
      std::make_unique<char[]>(defaults::SECTOR_LEN);
  ...
  uint32_t *nhood_buf =
      (uint32_t *)(node_buf.get() + (ndims_64 * sizeof(T)) + sizeof(uint32_t));
  ...
  if (append_reorder_data) {
    n_data_nodes_per_sector =
        defaults::SECTOR_LEN / (ndims_reorder_file * sizeof(float));
    n_reorder_sectors =
        ROUND_UP(npts_64, n_data_nodes_per_sector) / n_data_nodes_per_sector;
  }
  uint64_t disk_index_file_size =
      (n_sectors + n_reorder_sectors + 1) * defaults::SECTOR_LEN;
  ...
  if (nnodes_per_sector > 0) {  // Write multiple nodes per sector
    for (uint64_t sector = 0; sector < n_sectors; sector++) {
      ...
      for (uint64_t sector_node_id = 0;
        ...
        // read node's nhood
        vamana_reader.read(
            (char *)nhood_buf, (std::min)(nnbrs, width_u32) * sizeof(uint32_t));
        if (nnbrs > width_u32) {
          vamana_reader.seekg(
              (nnbrs - width_u32) * sizeof(uint32_t), vamana_reader.cur);
        }
        ...
        // write nnbrs
        *(uint32_t *)(node_buf.get() + ndims_64 * sizeof(T)) =
            (std::min)(nnbrs, width_u32);

        // write nhood next
        memcpy(
            node_buf.get() + ndims_64 * sizeof(T) + sizeof(uint32_t), nhood_buf,
    ...
    uint64_t nsectors_per_node =
        DIV_ROUND_UP(max_node_len, defaults::SECTOR_LEN);
    for (uint64_t i = 0; i < npts_64; i++) {
      if ((i * nsectors_per_node) % 100000 == 0) {
        diskann::cout << "Sector #" << i * nsectors_per_node << "written"
                      << std::endl;
      }
      memset(
          multisector_buf.get(), 0, nsectors_per_node * defaults::SECTOR_LEN);
      ...
      base_reader.read((char *)cur_node_coords.get(), sizeof(T) * ndims_64);
      memcpy(
          multisector_buf.get(), cur_node_coords.get(), ndims_64 * sizeof(T));
      ...
      *(uint32_t *)(multisector_buf.get() + ndims_64 * sizeof(T)) =
      ...
      // write nhood next
      memcpy(
          multisector_buf.get() + ndims_64 * sizeof(T) + sizeof(uint32_t),
          nhood_buf, (std::min)(nnbrs, width_u32) * sizeof(uint32_t));

      // flush sector to disk
      diskann_writer.write(
          multisector_buf.get(), nsectors_per_node * defaults::SECTOR_LEN);
    }
  }
  ...
  if (append_reorder_data) {
    ...
    for (uint64_t sector = 0; sector < n_reorder_sectors; sector++) {
      ...
      for (uint64_t sector_node_id = 0;
           sector_node_id < n_data_nodes_per_sector && sector_node_id < npts_64;
           sector_node_id++) {
        ...
        // copy node buf into sector_node_buf
        memcpy(
            sector_buf.get() + (sector_node_id * vec_len), vec_buf.get(),
            vec_len);
      }
      // flush sector to disk
      diskann_writer.write(sector_buf.get(), defaults::SECTOR_LEN);
  ...
  diskann::save_bin<uint64_t>(
      output_file, output_file_meta.data(), output_file_meta.size(), 1, 0);
...
int build_disk_index(
  ...
  if (!std::is_same<T, float>::value &&
      (compareMetric == diskann::Metric::INNER_PRODUCT ||
       compareMetric == diskann::Metric::COSINE)) {
    std::stringstream stream;
    stream << "Disk-index build currently only supports floating point data "
              "for Max "
              "Inner Product Search/ cosine similarity. "
  ...
  if (param_list.size() > 5) {
    ...
    use_disk_pq = true;
    if (disk_pq_dims == 0) use_disk_pq = false;
  }
  ...
  if (use_filters) {
    convert_labels_string_to_int(
        labels_file_original, labels_file_to_use, disk_labels_int_map_file,
        universal_label);
  ...
  // const double p_val = ((double)MAX_PQ_TRAINING_SET_SIZE /
  // (double)points_num);
  const double p_val = ((double)KMEANS_SAMPLE_SIZE / (double)points_num);

  if (use_disk_pq) {
    generate_disk_quantized_data<T>(
        data_file_to_use, disk_pq_pivots_path, disk_pq_compressed_vectors_path,
  ...
  generate_quantized_data<T>(
      ...
      compareMetric, p_val, num_pq_chunks, use_opq, codebook_prefix);
...
// This code should only be available in the "build" DLL.
  ...
  timer.reset();
  if (!use_disk_pq) {
    diskann::create_disk_layout<T>(
        data_file_to_use.c_str(), mem_index_path, disk_index_path);
  } else {
    if (!reorder_data)
      diskann::create_disk_layout<uint8_t>(
          disk_pq_compressed_vectors_path, mem_index_path, disk_index_path);
    else
      diskann::create_disk_layout<uint8_t>(
          disk_pq_compressed_vectors_path, mem_index_path, disk_index_path,
  ...
  gen_random_slice<T>(
      data_file_to_use.c_str(), sample_base_prefix, sample_sampling_rate);
  if (use_filters) {
  ...
  if (created_temp_file_for_processed_data) std::remove(prepped_base.c_str());
  std::remove(mem_index_path.c_str());
  if (use_disk_pq) std::remove(disk_pq_compressed_vectors_path.c_str());
...
template <typename T, typename LabelT>
int build_disk_index_4_merge(
    const char *dataFilePath, const char *indexFilePath,
    const char *indexBuildParameters, diskann::Metric compareMetric,
    bool use_opq, const std::string &codebook_prefix, bool use_filters,
    const std::string &label_file, const std::string &universal_label,
    const uint32_t filter_threshold, const uint32_t Lf) {
  diskann::cout << "Executing build_disk_index_4_merge()" << std::endl;
  std::stringstream parser;
  parser << std::string(indexBuildParameters);
  std::string cur_param;
  std::vector<std::string> param_list;
...
// This code should only be available in the "build" DLL.
  ...
  // if (!use_disk_pq)
  // {
  //     diskann::create_disk_layout<T>(data_file_to_use.c_str(),
  //     mem_index_path, disk_index_path);
  // }
  // else
  // {
  //     if (!reorder_data)
  //         diskann::create_disk_layout<uint8_t>(disk_pq_compressed_vectors_path,
  //         mem_index_path, disk_index_path);
  //     else
  //         diskann::create_disk_layout<uint8_t>(disk_pq_compressed_vectors_path,
  //         mem_index_path, disk_index_path,
  ...
  //     ten_percent_points > MAX_SAMPLE_POINTS_FOR_WARMUP ?
  //     MAX_SAMPLE_POINTS_FOR_WARMUP : ten_percent_points;
  // double sample_sampling_rate = num_sample_points / points_num;
  // gen_random_slice<T>(data_file_to_use.c_str(), sample_base_prefix,
  // sample_sampling_rate); if (use_filters)
  // {
  //     copy_file(labels_file_to_use, disk_labels_file);
  //     std::remove(mem_labels_file.c_str());
  //     if (universal_label != "")
...
template DISKANN_DLLEXPORT int build_disk_index<float, uint16_t>(
    ...
    const std::string &label_file, const std::string &universal_label,
    const uint32_t filter_threshold, const uint32_t Lf);
template DISKANN_DLLEXPORT int build_disk_index_4_merge<int8_t, uint32_t>(
    const char *dataFilePath, const char *indexFilePath,
    const char *indexBuildParameters, diskann::Metric compareMetric,
    bool use_opq, const std::string &codebook_prefix, bool use_filters,
    const std::string &label_file, const std::string &universal_label,
    const uint32_t filter_threshold, const uint32_t Lf);
template DISKANN_DLLEXPORT int build_disk_index_4_merge<uint8_t, uint32_t>(
    const char *dataFilePath, const char *indexFilePath,
    const char *indexBuildParameters, diskann::Metric compareMetric,
    bool use_opq, const std::string &codebook_prefix, bool use_filters,
    const std::string &label_file, const std::string &universal_label,
    const uint32_t filter_threshold, const uint32_t Lf);
template DISKANN_DLLEXPORT int build_disk_index_4_merge<float, uint32_t>(
    const char *dataFilePath, const char *indexFilePath,
    const char *indexBuildParameters, diskann::Metric compareMetric,
    bool use_opq, const std::string &codebook_prefix, bool use_filters,
    const std::string &label_file, const std::string &universal_label,
    const uint32_t filter_threshold, const uint32_t Lf);
// LabelT = uint16
template DISKANN_DLLEXPORT int build_disk_index_4_merge<int8_t, uint16_t>(
    const char *dataFilePath, const char *indexFilePath,
    const char *indexBuildParameters, diskann::Metric compareMetric,
    bool use_opq, const std::string &codebook_prefix, bool use_filters,
    const std::string &label_file, const std::string &universal_label,
    const uint32_t filter_threshold, const uint32_t Lf);
template DISKANN_DLLEXPORT int build_disk_index_4_merge<uint8_t, uint16_t>(
    const char *dataFilePath, const char *indexFilePath,
    const char *indexBuildParameters, diskann::Metric compareMetric,
    bool use_opq, const std::string &codebook_prefix, bool use_filters,
    const std::string &label_file, const std::string &universal_label,
    const uint32_t filter_threshold, const uint32_t Lf);
template DISKANN_DLLEXPORT int build_disk_index_4_merge<float, uint16_t>(
    const char *dataFilePath, const char *indexFilePath,
    const char *indexBuildParameters, diskann::Metric compareMetric,
    bool use_opq, const std::string &codebook_prefix, bool use_filters,
    const std::string &label_file, const std::string &universal_label,
...
template DISKANN_DLLEXPORT int kmeans_partition<int8_t, uint32_t>(
    IndexParameter &index_param);
template DISKANN_DLLEXPORT int kmeans_partition<float, uint32_t>(
    IndexParameter &index_param);
template DISKANN_DLLEXPORT int kmeans_partition<uint8_t, uint32_t>(
    IndexParameter &index_param);
template DISKANN_DLLEXPORT int kmeans_partition<int8_t, uint16_t>(
    IndexParameter &index_param);
template DISKANN_DLLEXPORT int kmeans_partition<float, uint16_t>(
    IndexParameter &index_param);
template DISKANN_DLLEXPORT int kmeans_partition<uint8_t, uint16_t>(
    IndexParameter &index_param);

template DISKANN_DLLEXPORT int random_partition<int8_t, uint32_t>(
    IndexParameter &index_param);
template DISKANN_DLLEXPORT int random_partition<float, uint32_t>(
    IndexParameter &index_param);
template DISKANN_DLLEXPORT int random_partition<uint8_t, uint32_t>(
    IndexParameter &index_param);
template DISKANN_DLLEXPORT int random_partition<int8_t, uint16_t>(
    IndexParameter &index_param);
template DISKANN_DLLEXPORT int random_partition<float, uint16_t>(
    IndexParameter &index_param);
template DISKANN_DLLEXPORT int random_partition<uint8_t, uint16_t>(
    IndexParameter &index_param);

template DISKANN_DLLEXPORT void build_all_partition<int8_t, uint32_t>(
    IndexParameter &index_param);
template DISKANN_DLLEXPORT void build_all_partition<float, uint32_t>(
    IndexParameter &index_param);
template DISKANN_DLLEXPORT void build_all_partition<uint8_t, uint32_t>(
    IndexParameter &index_param);

template DISKANN_DLLEXPORT void build_local_partition<int8_t, uint32_t>(
    IndexParameter &index_param);
template DISKANN_DLLEXPORT void build_local_partition<float, uint32_t>(
    IndexParameter &index_param);
template DISKANN_DLLEXPORT void build_local_partition<uint8_t, uint32_t>(
    IndexParameter &index_param);
template DISKANN_DLLEXPORT void build_local_partition<int8_t, uint16_t>(
    IndexParameter &index_param);
template DISKANN_DLLEXPORT void build_local_partition<float, uint16_t>(
    IndexParameter &index_param);
template DISKANN_DLLEXPORT void build_local_partition<uint8_t, uint16_t>(
    IndexParameter &index_param);
